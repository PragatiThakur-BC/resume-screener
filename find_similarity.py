...
# Function to select multiple resume PDF files
def select_resumes():
    st.header("Select Resumes (PDF)")
    uploaded_files = st.file_uploader("Upload up to 50 resume PDF files", accept_multiple_files=True, type=["pdf"])
...
def main():
    ...
    # Screen Resumes button with custom CSS for size
    st.markdown(
        """
        <style>
        .stButton>button {
...
def display_ranking_table(uploaded_files, jd_text):
    ...
    for rank, resume_file in enumerate(uploaded_files, start=1):
        try:
            ...
            st.write(f"Not a pdf file,Recheck the file : {resume_file.name}")
